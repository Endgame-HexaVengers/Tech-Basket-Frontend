"use client";

import React from "react";
import { X, Printer, CheckCircle, ArrowDownLeft, Building2 } from "lucide-react";
import { PurchaseReturn } from "@/types/purchase";

interface Props {
  isOpen: boolean;
  onClose: () => void;
  returnData: PurchaseReturn | null;
}

export default function ReturnDebitNoteModal({ isOpen, onClose, returnData }: Props) {
  if (!isOpen || !returnData) return null;

  const handlePrint = () => {
    window.print();
  };

  const getSettlementLabel = (type?: string) => {
    switch (type) {
      case "adjust_due":
        return "Adjusted with Supplier Due Balance";
      case "supplier_credit":
        return "Supplier Credit Ledger (Future Purchase)";
      case "cash_refund":
        return "Direct Cash / Bank Refund Received";
      default:
        return "Supplier Credit Note";
    }
  };

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm print:p-0 print:bg-white animate-in fade-in overflow-y-auto">
      {/* Container */}
      <div className="relative my-6 flex w-full max-w-3xl flex-col rounded-2xl bg-white shadow-2xl border border-slate-200 print:border-none print:shadow-none print:my-0">
        {/* Actions Bar (hidden in print) */}
        <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50/80 px-6 py-3.5 print:hidden">
          <div className="flex items-center gap-2">
            <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-emerald-100 text-emerald-700">
              <CheckCircle className="h-4 w-4" />
            </span>
            <div>
              <h2 className="text-sm font-bold text-slate-800">Debit Note Generated</h2>
              <p className="text-xs text-slate-500">Official supplier return voucher</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={handlePrint}
              className="inline-flex items-center gap-2 rounded-lg bg-[#123b9c] px-4 py-2 text-xs font-semibold text-white shadow-sm hover:bg-blue-800 transition"
            >
              <Printer className="h-4 w-4" />
              Print Debit Note
            </button>
            <button
              onClick={onClose}
              className="rounded-lg p-2 text-slate-400 hover:bg-slate-200 hover:text-slate-700 transition"
            >
              <X className="h-5 w-5" />
            </button>
          </div>
        </div>

        {/* Printable Paper Area */}
        <div className="p-8 print:p-6 text-slate-800 bg-white" id="debit-note-print-area">
          {/* Company & Voucher Header */}
          <div className="flex flex-col sm:flex-row justify-between items-start gap-4 pb-6 border-b-2 border-slate-800">
            <div>
              <div className="flex items-center gap-2">
                <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#123b9c] text-white font-extrabold text-xl shadow-md">
                  TB
                </div>
                <div>
                  <h1 className="text-xl font-black tracking-tight text-slate-900">TECH BASKET</h1>
                  <p className="text-xs text-slate-500 font-medium">Retail & Inventory Management System</p>
                </div>
              </div>
              <p className="text-xs text-slate-500 mt-2">
                Corporate Office: Level 4, Tech Plaza, Dhaka, Bangladesh<br />
                Phone: +880 1700-000000 | Email: support@techbasket.com
              </p>
            </div>

            <div className="text-left sm:text-right">
              <span className="inline-block rounded bg-red-100 px-3 py-1 text-xs font-bold uppercase tracking-wider text-red-800 mb-1">
                DEBIT NOTE / RETURN VOUCHER
              </span>
              <p className="font-mono text-base font-bold text-slate-900 mt-1">
                {returnData.returnId}
              </p>
              <p className="text-xs text-slate-500">
                Date: {new Date(returnData.returnDate || returnData.createdAt).toLocaleDateString("en-GB", {
                  year: "numeric",
                  month: "short",
                  day: "numeric",
                })}
              </p>
            </div>
          </div>

          {/* Supplier & Purchase Meta */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 py-4 border-b border-slate-200 text-xs">
            <div className="rounded-xl bg-slate-50 p-3.5 border border-slate-200/80">
              <span className="font-semibold text-slate-500 uppercase tracking-wider block mb-1">
                Debit To (Supplier):
              </span>
              <p className="text-sm font-bold text-slate-900">{returnData.supplierName || "Supplier"}</p>
              {returnData.supplierPhone && (
                <p className="text-slate-600 mt-0.5">Phone: {returnData.supplierPhone}</p>
              )}
              {returnData.branchName && (
                <p className="text-slate-600 mt-0.5 flex items-center gap-1">
                  <Building2 className="h-3 w-3 text-slate-400" /> Branch: {returnData.branchName}
                </p>
              )}
            </div>

            <div className="rounded-xl bg-slate-50 p-3.5 border border-slate-200/80 space-y-1">
              <div className="flex justify-between">
                <span className="text-slate-500 font-medium">Original Invoice No:</span>
                <span className="font-mono font-bold text-blue-900">{returnData.purchaseInvoiceNo || returnData.purchaseId}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500 font-medium">Settlement Method:</span>
                <span className="font-semibold text-slate-800">{getSettlementLabel(returnData.settlementType)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500 font-medium">Voucher Status:</span>
                <span className="font-bold text-emerald-700">{returnData.status || "Completed"}</span>
              </div>
            </div>
          </div>

          {/* Table of Returned Items */}
          <div className="mt-6 overflow-hidden rounded-xl border border-slate-200">
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-100/90 text-slate-600 font-bold border-b border-slate-200">
                <tr>
                  <th className="py-3 px-3 w-10 text-center">#</th>
                  <th className="py-3 px-3">Product Description</th>
                  <th className="py-3 px-3">Return Reason</th>
                  <th className="py-3 px-3 text-center">Qty</th>
                  <th className="py-3 px-3 text-right">Unit Price</th>
                  <th className="py-3 px-3 text-right">Credit Amount</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {returnData.items.map((item, idx) => (
                  <tr key={idx} className="hover:bg-slate-50/50">
                    <td className="py-3 px-3 text-center text-slate-400">{idx + 1}</td>
                    <td className="py-3 px-3 font-semibold text-slate-800">
                      {item.productName || item.title || "Product"}
                      {item.serialNumbers && item.serialNumbers.length > 0 && (
                        <div className="text-[10px] font-normal text-slate-500 mt-0.5">
                          S/N: {item.serialNumbers.join(", ")}
                        </div>
                      )}
                    </td>
                    <td className="py-3 px-3 text-slate-600">
                      <span className="inline-block px-2 py-0.5 rounded bg-slate-100 text-slate-700 text-[11px]">
                        {item.returnReason || returnData.returnReason || "General Return"}
                      </span>
                    </td>
                    <td className="py-3 px-3 text-center font-bold text-slate-800">
                      {item.quantity || item.quantityReturned || 1}
                    </td>
                    <td className="py-3 px-3 text-right text-slate-600">
                      ৳ {Number(item.price || item.unitPrice || 0).toLocaleString()}
                    </td>
                    <td className="py-3 px-3 text-right font-bold text-slate-900">
                      ৳ {Number(item.refundAmount || 0).toLocaleString()}
                    </td>
                  </tr>
                ))}
              </tbody>
              <tfoot className="border-t-2 border-slate-300 bg-slate-50 font-bold">
                <tr>
                  <td colSpan={3} className="py-3 px-4 text-slate-600">Total Returned Items</td>
                  <td className="py-3 px-3 text-center text-slate-900">
                    {returnData.items.reduce((s, i) => s + Number(i.quantity || i.quantityReturned || 1), 0)}
                  </td>
                  <td className="py-3 px-3 text-right text-slate-600">Net Credit:</td>
                  <td className="py-3 px-3 text-right text-sm text-blue-900">
                    ৳ {Number(returnData.totalRefundAmount || 0).toLocaleString()}
                  </td>
                </tr>
              </tfoot>
            </table>
          </div>

          {/* Notes / Reason */}
          {returnData.notes && (
            <div className="mt-4 rounded-lg bg-amber-50 p-3 border border-amber-200 text-xs text-amber-900">
              <span className="font-bold">Remarks / Notes:</span> {returnData.notes}
            </div>
          )}

          {/* Signatures */}
          <div className="mt-14 grid grid-cols-3 gap-8 text-center text-xs text-slate-500 pt-6">
            <div className="border-t border-slate-300 pt-2">
              <p className="font-semibold text-slate-700">Prepared By</p>
              <p className="text-[10px] text-slate-400 mt-0.5">Store Executive</p>
            </div>
            <div className="border-t border-slate-300 pt-2">
              <p className="font-semibold text-slate-700">Verified & Approved</p>
              <p className="text-[10px] text-slate-400 mt-0.5">Store Manager</p>
            </div>
            <div className="border-t border-slate-300 pt-2">
              <p className="font-semibold text-slate-700">Supplier / Recipient</p>
              <p className="text-[10px] text-slate-400 mt-0.5">Signature & Date</p>
            </div>
          </div>
        </div>

        {/* Footer (hidden in print) */}
        <div className="flex items-center justify-between border-t border-slate-100 bg-slate-50/80 px-6 py-3.5 print:hidden">
          <p className="text-xs text-slate-400">
            This debit note confirms return of inventory to supplier and credit adjustment.
          </p>
          <button
            onClick={onClose}
            className="rounded-lg bg-slate-200 px-4 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-300 transition"
          >
            Close
          </button>
        </div>
      </div>
    </div>
  );
}
